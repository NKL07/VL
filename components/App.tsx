...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-vl-dark text-white font-sans selection:bg-vl-accent selection:text-black">
      ...
      {/* Refined Global Navigation Bar */}
      <header 
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-500 ease-in-out border-b ${
          currentView === 'HOME' && !isScrolled
            ? 'bg-transparent border-transparent py-6 text-shadow-lg' // Transparent at top of home
            : 'bg-[#030304]/90 backdrop-blur-xl border-white/5 py-3 shadow-[0_4px_30px_rgba(0,0,0,0.3)]' // Solid/Glass elsewhere
        }`}
      >
        <div className="container mx-auto px-6 flex justify-between items-center relative">
          
          {/* Left Side: Dynamic Logo or Back Button */}
              ...
              {currentView === 'HOME' ? (
                  ...
                  >
                      ...
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 ${
                          !isScrolled 
                            ? 'bg-vl-accent text-black shadow-[0_0_20px_rgba(234,179,8,0.4)] scale-105' // Pop out more on hero
                            : 'bg-white/10 text-white border border-white/10'
                      }`}>
                          <span className="font-bold font-mono text-lg tracking-tighter">VL</span>
                      </div>
                      
                      {/* Text */}
                      <div className={`flex flex-col items-start transition-all duration-300 ${isScrolled ? 'opacity-100' : 'opacity-90 text-shadow-md'}`}>
                          <span className="font-bold text-white text-sm tracking-tight leading-none">RENT A CAR</span>
                          <span className={`text-[10px] uppercase tracking-widest font-mono ${!isScrolled ? 'text-gray-200' : 'text-gray-400'}`}>Matara</span>
                      </div>
                  </button>
              ) : (
                  <button 
                      ...
                      className="group flex items-center gap-3 focus:outline-none"
                  >
                      <div className="w-10 h-10 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center text-white transition-all duration-300 group-hover:bg-vl-accent group-hover:text-black group-hover:border-vl-accent">
                          <ArrowLeft size={20} className="transition-transform duration-300 group-hover:-translate-x-1" />
                      </div>
                      <span className="font-bold text-white text-sm uppercase tracking-widest hidden sm:block group-hover:text-vl-accent transition-colors">Back</span>
                  </button>
              )}
          </div>

          {/* Center: Dynamic Page Title (Fades in when scrolled or on subpages) */}
          <div className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transition-all duration-500 pointer-events-none ${
              isScrolled || currentView !== 'HOME' ? 'opacity-100 translate-y-[-50%]' : 'opacity-0 translate-y-[-20%]'
          }`}>
              <span className="font-mono text-xs font-bold text-gray-500 uppercase tracking-[0.3em] hidden md:block">
                  {currentView === 'HOME' ? 'Premium Fleet' : currentView.replace('_', ' ')}
              </span>
          </div>
          ...
          <div className="relative z-10">
            <button 
                ...
                className={`group flex items-center gap-3 px-4 py-2 rounded-full border transition-all duration-300 hover:scale-105 active:scale-95 ${
                    currentView === 'HOME' && !isScrolled
                        ? 'bg-black/30 border-white/20 hover:bg-black/50 text-white backdrop-blur-md shadow-lg' // Increased visibility on hero
                        : 'bg-vl-surface border-vl-subtle hover:border-vl-accent/50 text-gray-300 hover:text-white'
                }`}
            >
                <span className="font-mono text-xs font-bold uppercase tracking-widest hidden sm:block">
                    {user ? user.firstName : 'Menu'}
                </span>
                <Menu size={20} className={`${currentView === 'HOME' && !isScrolled ? 'text-vl-accent' : 'text-gray-400 group-hover:text-vl-accent'} transition-colors`} />
            </button>
          </div>
        </div>
      </header>
      ...
      {/* Main Content Router */}
      <main className={`relative z-10 min-h-screen ${currentView === 'HOME' ? '' : 'pt-24'}`}>
        {currentView === 'HOME' && (
          <>
